...
def update_bar_graph(hov_data, clk_data, slct_data, country_chosen):
    if hov_data is None:
        dff3 = df[df.country.isin(country_chosen)]
        fig3 = px.bar(data_frame=dff3, x='year', y='pop', color='country', title= 'pop per year',hover_data=["lifeExp", "pop", "iso_alpha"])
        fig3.update_traces()
        return fig3
    else:
        print(f'hover data: {hov_data}')
        ...
        fig3 = px.bar(data_frame=dff3, x='year', y='pop', title=f'pop per year: {hov_year}',color='country')
        fig3.update_traces()
        return fig3
